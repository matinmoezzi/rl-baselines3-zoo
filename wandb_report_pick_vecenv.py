...
report = wr.Report(
    ...
    title=f"Pick-and-Place {env_version} Task Experiments Vectorized Env",
    description="MyCobot Deep RL Experiments - Pick-and-Place Task - SAC, TQC - Vectorized Env",
    width="fluid"
)
...
for controller, reward, env in itertools.product(controllers, rewards, envs):
    ...
    runset = wr.runset.Runset("matinmoezzi", "mycobot-deepRL", query=q).set_filters_with_python_expr("n_timesteps == 6000000")
